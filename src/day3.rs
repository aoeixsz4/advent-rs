...
fn get_bit_counts(data: &[BitVec<Msb0, u16>]) -> Vec<usize> {
    (0..NR_BITS)
        .map(|i| data.iter().filter(|bit_vec| bit_vec[i]).count())
        .collect()
}

fn get_bit_count(data: &[BitVec<Msb0, u16>], index: usize) -> usize {
...
fn filter_bitfields(bitfields: &Vec<BitVec<Msb0, u16>>, popular: bool) -> i64 {
    ...
    for i in 0..NR_BITS {
        ...
        if sublen == 1 {
            return (bitfields_subset[0].as_raw_slice()[0] >> (16 - NR_BITS)) as i64;
        }
        let bit_count = get_bit_count(&bitfields_subset, i);
        bitfields_subset = bitfields_subset
            .into_iter()
            .filter(|bit_vector| {
                let test = if popular {
                    bit_vector[i]
